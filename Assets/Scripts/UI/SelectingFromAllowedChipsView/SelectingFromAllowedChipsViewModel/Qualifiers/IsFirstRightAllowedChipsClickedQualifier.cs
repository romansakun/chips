...
namespace UI
{
    public class IsFirstRightAllowedChipsClickedQualifier : BaseSelectingFromAllowedChipsViewModelQualifier
    {
        ...
        protected override float Score(SelectingFromAllowedChipsViewModelContext context)
        {
            ...
            if (context.RightSideChips.IndexOf(chip) != 0)
                return 0;

            Debug.Log($"hit obj: {hitObject.name}", hitObject.gameObject);
