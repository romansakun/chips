    ...
    public class AnimateMovingAndRotationAllChipsAction : BaseSelectingFromAllowedChipsViewModelAction
    {
        ...
        public override async Task ExecuteAsync(SelectingFromAllowedChipsViewModelContext context)
        {
            ...
            if (context.CurrentWatchingChip != default)
            {
                ...
                _sequence
                    .Join(chipFacade.Transform.DOMove(watchPosition, duration))
                    .Join(chipFacade.Transform.DORotate(watchRotation, duration));
            }
            for (var i = 0; i < context.RightSideChips.Count; i++)
            {
                var chipFacade = context.RightSideChips[i].Item1.Facade;
                _sequence.Join(chipFacade.Transform.DOMove(rightPosition + rightOffset * i, duration));
                _sequence.Join(chipFacade.Transform.DORotate(rightRotation, duration));
            }  
            for (var i = 0; i < context.LeftSideChips.Count; i++)
            {
                var chipFacade = context.LeftSideChips[i].Item1.Facade;
                _sequence.Join(chipFacade.Transform.DOMove(leftPosition + leftOffset * i, duration));
                _sequence.Join(chipFacade.Transform.DORotate(leftRotation, duration));
            }
            for (var i = 0; i < context.BetSelectedChips.Count; i++)
            {
                var chipFacade = context.BetSelectedChips[i].Item1.Facade;
                _sequence.Join(chipFacade.Transform.DOMove(betPosition + betOffset * i, duration));
                _sequence.Join(chipFacade.Transform.DORotate(betRotation, duration));
            }
